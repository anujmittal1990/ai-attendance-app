...
// Set high body parser limits for face vector array payloads
...
// Database Connection Pool to handle high-concurrency requests safely
...
// Test initial database connection pool
db.getConnection((err, connection) => {
    if (err) {
        console.error('Database pool connection initialization error:', err.message);
    } else {
...
/**
 * 1. REGISTER EMPLOYEE & FACE DESCRIPTOR
 */
app.post('/api/employees', (req, res) => {
    ...
    db.query(query, [userId, name, descriptorString], (err) => {
        if (err) {
            console.error('Error saving employee profile:', err.message);
...
app.get('/api/employees', (req, res) => {
    ...
    db.query(query, (err, results) => {
        ...
        try {
            ...
            console.error('Error parsing employee face descriptors:', parseErr.message);
...
app.post('/api/attendance', (req, res) => {
    ...
    db.query(checkQuery, [userId, userId], (err, results) => {
        ...
        db.query(insertQuery, [userId], (err) => {
            if (err) {
                console.error('Attendance insertion error:', err.message);
...
/**
 * 4. FETCH ATTENDANCE LOGS (SAFE EXECUTION ROUTE)
 */
app.get('/api/attendance-logs', (req, res) => {
    ...
    let query = `
        SELECT 
            a.id, 
            a.user_id, 
            COALESCE(e.name, 'Unknown Employee') AS name, 
            a.timestamp 
        FROM attendance a
...
// Global Safety Exception Handlers (Prevents Server Crash and 503 Errors)
